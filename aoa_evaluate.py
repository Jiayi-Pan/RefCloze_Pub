...
def eval_one_batch(model, batch_in, device='cuda', amp=False):
	...
	for dp_idx in range(len(infos)):
		this_raw = {"dpid": dp_ids[dp_idx]}
		# keep only predictions with 0.7+ confidence
		probas = 1 - model_outputs['pred_logits'].float().softmax(-1)[dp_idx, :, -1].cpu()
		keep = (probas > 0.7).cpu()

		# convert boxes from [0; 1] to image scales
		bboxes_scaled = rescale_bboxes(model_outputs['pred_boxes'].float()[dp_idx, keep].cpu(), infos[dp_idx]['text_info']['img_size'])
		# Extract the text spans predicted by each box
		positive_tokens = (model_outputs["pred_logits"].float()[dp_idx, keep].softmax(-1).cpu() > 0.1).nonzero().tolist()
		...
		GT_bboxs = [box_coco_to_xyxy(infos[dp_idx]['bboxs'][0][i]) for i in range(len(infos[dp_idx]['bboxs'][0]))]
		...
		## MD metric
		phrase_token_idxs = mask_region_to_token_idxs(encs, batch_phrase_spans_after_mask[dp_idx], batch_idx = dp_idx)
		GT_Dis = pos_idxs_to_distribution(phrase_token_idxs, device=device)

		# find ones that has highest confidence on grounding to mask token
		vis_text_span_predictions = model_outputs["pred_logits"].float()[dp_idx, :].softmax(-1).cpu()
		...
		## Cal these scores 
		this_dis = vis_text_span_predictions[most_confident_raw_idx]
		this_box = rescale_bboxes(model_outputs['pred_boxes'].float().cpu()[dp_idx, most_confident_raw_idx:most_confident_raw_idx+1], infos[dp_idx]['text_info']['img_size']).to(device)
		this_normed_prob = interested_prob[most_confident_raw_idx]
		this_kl = math_KL_div(GT_Dis, this_dis)
		this_any_iou = calc_matching_score_ANY_PROTOCOL(GT_bboxs, this_box)
		this_raw['grounding'] = {}
		this_raw['grounding']['raw_top1'] = {'normed_prob': this_normed_prob,'kl': this_kl, 'any_iou': this_any_iou, 'bbox': this_box.tolist()}
		# note, last idx has special meaning
		md_idxs = deepcopy(interested_proposal_idxs)
		this_raw['grounding']["proposed"] = []
		for idx in range(len(md_idxs)):
			this_box = interested_bboxs[idx:idx+1]
			this_idx = interested_proposal_idxs[idx]
			this_dis = model_outputs["pred_logits"].float()[dp_idx, keep].softmax(-1)[this_idx].cpu()
			this_normed_prob = 0.
			for token_idx in phrase_token_idxs:
				this_normed_prob += this_dis[token_idx].item()
			this_kl = math_KL_div(GT_Dis, this_dis)
			this_any_iou = calc_matching_score_ANY_PROTOCOL(GT_bboxs, this_box)
			this_raw['grounding']["proposed"].append({'normed_prob': this_normed_prob, 'kl': this_kl, 'any_iou': this_any_iou})
		
		# softeval by bbox heatmap
		objectiveness_probs = 1 - model_outputs['pred_logits'].softmax(-1)[dp_idx, :, -1].cpu()
		lang_match_probs = torch.ones_like(model_outputs["pred_logits"][dp_idx].cpu()[:,0])
		normed_pred_logits = model_outputs["pred_logits"][dp_idx].softmax(dim=-1).cpu()
		for interested_idx in interested_idxs:
			lang_match_probs *= normed_pred_logits[:, interested_idx]
		bbox_probs = lang_match_probs * objectiveness_probs 
		img_size = infos[dp_idx]['text_info']['img_size']
		bboxes = rescale_bboxes(model_outputs['pred_boxes'].cpu()[dp_idx,:],img_size)
		pred_M = create_heat_map(img_size, bboxes, bbox_probs)
		gt_M = create_heat_map(img_size, GT_bboxs, torch.ones_like(GT_bboxs[:,0]))
		soft_eval_l1 = torch.nn.functional.l1_loss(pred_M.cuda().flatten(), gt_M.cuda().flatten())
		soft_eval_l2 = torch.nn.functional.mse_loss(pred_M.cuda().flatten(), gt_M.cuda().flatten())
		soft_eval_kl = torch.nn.functional.kl_div(pred_M.cuda().flatten(), gt_M.cuda().flatten(), reduction='batchmean')

		embeds = []
		for idx in interested_idxs:
			# code.interact(local=locals())
			embeds.append(model_outputs['lang_embeddings'][dp_idx, idx].cpu().tolist())
		word_embedding = np.mean(embeds, axis=0)
		word = infos[dp_idx]['text_info']['mask_regions'][0][1]
		Word_Embedding_Dic[word].append(word_embedding)

		hit_at_x = None
		this_raw['ppl'] = float(ppl_score) 
		this_raw['num_token'] = len(interested_idxs)
		this_raw['rank'] = hit_at_x
		this_raw['any_iou'] = any_score
		this_raw['all_iou'] = float(all_score)
		this_raw['soft_match_l1'] = float(soft_eval_l1)
		this_raw['soft_match_l2'] = float(soft_eval_l2)
		this_raw['soft_match_kl'] = float(soft_eval_kl)
