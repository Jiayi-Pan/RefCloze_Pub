...
def eval_one_result(result, slicings, verbose = False):
    ...
    for slicing in slicings: 
        ...
        this_stats['bridge_raw_top1_norm_prob'] = mean(bridge['raw_top1']['normed_probs']).item()
        this_stats['bridge_raw_top1_kl'] = mean(bridge['raw_top1']['kls']).item()
...
def eval_one_result_with_error_handling(result, slicings, verbose=False):
    try:
        return eval_one_result(result, slicings, verbose)
    except Exception as e:
        print(f"Failed to eval {result}: {e}")
        return None
...
def AOA_eval(result_path, slicings, threads=16):
    ...
    with mp.Pool(threads) as p:
        results = list(tqdm(p.imap(partial(eval_one_result_with_error_handling, slicings=slicings, verbose=False), full_paths), total=len(full_paths)))
    ...
    ResultSummary = {}
    stat_names = ["ppl", "all_iou", "any_iou", "g_score_all", "g_score_any", "bridge_raw_top1_norm_prob", "bridge_raw_top1_kl", "bridge_raw_top1_any_iou", "bridge_raw_top1_any_box_hit","bridge_proposed_norm_prob", "bridge_proposed_kl", "bridge_proposed_any_iou", "mask_hit", "all_box_hit", "all_hit", "any_box_hit", "any_hit", "soft_match_l1", "soft_match_l2", "soft_match_kl", "all_hit_top3", "all_hit_top5", "all_hit_top10", "any_hit_top3", "any_hit_top5", "any_hit_top10", "word_dis", "amb_any_iou", "amb_any_box_hit", "mask_hit_top3", "mask_hit_top5", "mask_hit_top10", "embedding"]
